...
class ExampleModel(nn.Module):
    def __init__(
        ...
        self.audio_net = Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(in_features=n_audio_feats // 8, out_features=fc_hidden),
        )
        self.video_net = Sequential(
            nn.Conv2d(
                in_channels=n_video_channels,
                out_channels=n_hidden_channels,
                kernel_size=4,
                stride=2,
                padding=1,
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(in_features=(n_video_feats // 8) ** 2, out_features=fc_hidden),
        )
    ...
    def forward(self, mix_audio, s_video, s_audio_length, **batch):
        ...
        return {
            ...
            // 128,  # we changed the resolution to x128 lower
        }
