...
from src.model.rtfsnet.layers import activations, conv_layers, normalizations
from src.model.rtfsnet.layers.attention import MultiHeadSelfAttention
...
class DualPathRNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.rnn_out_chan = (
            self.hid_chan * self.num_direction
            if self.rnn_type != "Attn"
            else self.unfolded_chan
        )
        ...
        self.ffn = (
            conv_layers.FeedForwardNetwork(
                self.unfolded_chan,
                self.unfolded_chan * 2,
                self.kernel_size,
                dropout=0.1,
            )
            if self.apply_ffn
        ...
        self.linear = nn.ConvTranspose1d(
            self.rnn_out_chan, self.in_chan, self.kernel_size, stride=self.stride
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        new_F = (
            math.ceil((old_F - self.kernel_size) / self.stride) * self.stride
            + self.kernel_size
        )
        ...
        **kwargs,
        ...
        self.linear_ih = nn.Sequential(
            conv_layers.ConvActNorm(
                self.in_chan, self.in_chan, self.kernel_size, groups=self.in_chan
            ),
        ...
        if self.num_directions > 1:
            ...
            self.linear_hh_b = conv_layers.ConvActNorm(
                self.hid_chan, 4 * self.hid_chan, 1
            )

    def forward(
        self, input: torch.Tensor, hidden_t: torch.Tensor, cell_t: torch.Tensor
    ):
        # x has size: (B, C, L)
        ...
        if self.num_directions > 1:
            ...
            gates_b = (
                self.linear_ih_b(input_b) + self.linear_hh_b(hidden_t_b)[:batch_size]
            )
...
class BiLSTM2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.lstm_cell = ConvLSTMCell(
            self.in_chan * self.window, self.hid_chan, self.kernel_size, self.num_dir
        )
        ks = (self.window, 1)
        s = (self.stride, 1)
        ...
        self.projection = nn.Sequential(
            ...
            conv_layers.ConvActNorm(
                self.hid_chan * self.num_dir, self.in_chan, 1, is2d=True
            ),
        )
    ...
    def pad(self, x: torch.Tensor):
        ...
        new_w = (
            math.ceil((old_w - self.window) / self.stride) * self.stride + self.window
        )
        new_h = (
            math.ceil((old_h - self.window) / self.stride) * self.stride + self.window
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(iterations):
            ...
            w, h = x_slice.shape[-2:]
            x_slice = (
                x_slice.permute(0, 3, 1, 2)
                .contiguous()
                .view(bs * h, self.in_chan * self.num_dir, w, 1)
            )
            x_slice = self.unfold(x_slice)
            hidden_t, cell_t = self.lstm_cell(x_slice, hidden_t, cell_t)
            outputs[i] = (
                hidden_t.view(bs, h, self.hid_chan * self.num_dir, -1)
                .permute(0, 2, 3, 1)
                .contiguous()
            )
...
class GlobalAttentionRNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.RNN = RNNProjection(
            self.in_chan, self.hid_chan, self.rnn_type, self.dropout, self.bidirectional
        )
...
class GlobalGALR(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.time_RNN = RNNProjection(
            self.in_chan, self.in_chan, self.rnn_type, self.dropout, self.bidirectional
        )
        self.freq_MHSA = MultiHeadSelfAttention(
            self.in_chan, self.n_head, self.dropout, self.pos_enc
        )
        self.freq_FFN = conv_layers.get(ffn_name)(
            self.in_chan, self.hid_chan, self.kernel_size, dropout=dropout
        )

        self.group_FFN = nn.Identity()
        if self.group_ffn:
            self.group_FFN = conv_layers.FeedForwardNetwork(
                self.in_chan,
                self.hid_chan,
                self.kernel_size,
                dropout=dropout,
