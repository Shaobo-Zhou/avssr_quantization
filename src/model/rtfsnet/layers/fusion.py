...
from src.model.rtfsnet.layers.conv_layers import ConvNormAct
...
class InjectionMultiSum(nn.Module):
    ...
    def forward(self, local_features: torch.Tensor, global_features: torch.Tensor):
        ...
        if torch.prod(torch.tensor(new_shape)) > torch.prod(torch.tensor(old_shape)):
            global_emb = F.interpolate(
                self.global_embedding(global_features), size=new_shape, mode="nearest"
            )
            gate = F.interpolate(
                self.global_gate(global_features), size=new_shape, mode="nearest"
            )
        else:
...
class ConvLSTMFusionCell(nn.Module):
    ...
    def forward(self, tensor_a: torch.Tensor, tensor_b: torch.Tensor):
        ...
        if torch.prod(torch.tensor(new_shape)) > torch.prod(torch.tensor(old_shape)):
            gates = self.conv_a(tensor_a) + F.interpolate(
                self.conv_b(tensor_b), size=new_shape, mode="nearest"
            )
        else:
            gates = self.conv_a(tensor_a) + self.conv_b(
                F.interpolate(tensor_b, size=new_shape, mode="nearest")
            )
...
class ATTNFusionCell(nn.Module):
    ...
    def forward(self, tensor_a: torch.Tensor, tensor_b: torch.Tensor):
        ...
        b_transformed = F.interpolate(
            self.resize(tensor_b), size=time_steps, mode="nearest"
        )
        if self.is2d:
