...
from src.model.rtfsnet.layers import conv_layers
...
class PositionalEncoding(nn.Module):
    def __init__(self, channels: int, max_len: int = 10000, *args, **kwargs):
        ...
        div_term = torch.exp(
            torch.arange(0, self.channels, 2).float()
            * -(torch.log(torch.tensor(self.max_len).float()) / self.channels)
        )
...
class GlobalAttention2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.time_MHSA = MultiHeadSelfAttention(
            self.in_chan, self.n_head, self.dropout, self.pos_enc
        )
        ...
        if self.single_ffn:
            self.time_FFN = conv_layers.get(self.ffn_name)(
                self.in_chan, self.hid_chan, self.kernel_size, dropout=dropout
            )
            self.freq_FFN = conv_layers.get(self.ffn_name)(
                self.in_chan, self.hid_chan, self.kernel_size, dropout=dropout
            )
...
class CoTAttention(nn.Module):
    def __init__(self, in_chan=512, kernel_size=3, *args, **kwargs):
        ...
        self.value_embed = nn.Sequential(
            nn.Conv2d(in_chan, in_chan, 1, bias=False), nn.BatchNorm2d(in_chan)
        )
