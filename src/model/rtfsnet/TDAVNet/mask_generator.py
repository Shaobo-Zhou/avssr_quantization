...
from src.model.rtfsnet.layers import ConvNormAct, activations
...
class MaskGenerator(BaseMaskGenerator):
    ...
    def __apply_masks(self, masks: torch.Tensor, audio_mixture_embedding: torch.Tensor):
        ...
        dims = audio_mixture_embedding.shape[
            -(len(audio_mixture_embedding.shape) // 2) :
        ]
        if self.RI_split:
            masks = masks.view(batch_size, self.n_src, 2, self.in_chan // 2, *dims)
            audio_mixture_embedding = audio_mixture_embedding.view(
                batch_size, 2, self.in_chan // 2, *dims
            )
            ...
            est_spec_real = (
                emb_real * mask_real - emb_imag * mask_imag
            )  # B, n_src, C/2, T, (F)
            est_spec_imag = (
                emb_real * mask_imag + emb_imag * mask_real
            )  # B, n_src, C/2, T, (F)

            separated_audio_embedding = torch.cat(
                [est_spec_real, est_spec_imag], 2
            )  # B, n_src, C, T, (F)
        else:
    ...
    def forward(
        self, refined_features: torch.Tensor, audio_mixture_embedding: torch.Tensor
    ):
        if self.direct:
            ...
            separated_audio_embedding = self.__apply_masks(
                masks, audio_mixture_embedding
            )
...
class MaskGenerator2Chan(BaseMaskGenerator):
    ...
    def forward(
        ...
        if self.direct:
            ...
            if self.output_gate:
                refined_features = self.output(refined_features) * self.gate(
                    refined_features
                )
