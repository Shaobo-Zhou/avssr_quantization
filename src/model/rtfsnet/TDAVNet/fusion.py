...
from src.model.rtfsnet.layers import (
    ATTNFusionCell,
    ConvGRUFusionCell,
    ConvLSTMFusionCell,
    ConvNormAct,
...
class ConcatFusion(FusionBasemodule):
    def __init__(
        ...
        super(ConcatFusion, self).__init__(
            ain_chan, vin_chan, kernel_size, video_fusion, is2d
        )

        self.audio_conv = ConvNormAct(
            self.ain_chan + self.vin_chan,
            self.ain_chan,
            self.kernel_size,
            norm_type="gLN",
            is2d=self.is2d,
    ...
    def forward(self, audio: torch.Tensor, video: torch.Tensor):
        ...
        video_interp = F.interpolate(
            video, size=audio.shape[-(len(audio.shape) // 2) :], mode="nearest"
        )
        ...
        if self.video_fusion:
            audio_interp = F.interpolate(
                audio, size=video.shape[-(len(video.shape) // 2) :], mode="nearest"
            )
...
class InjectionFusion(FusionBasemodule):
    def __init__(
        ...
        super(InjectionFusion, self).__init__(
            ain_chan, vin_chan, kernel_size, video_fusion, is2d
        )

        if video_fusion:
            self.audio_conv = ConvNormAct(
                self.ain_chan, self.vin_chan, 1, is2d=self.is2d
            )
            self.video_inj = InjectionMultiSum(
                self.vin_chan, self.kernel_size, "gLN", is2d=self.is2d
            )
        self.video_conv = ConvNormAct(self.vin_chan, self.ain_chan, 1, is2d=self.is2d)
        self.audio_inj = InjectionMultiSum(
            self.ain_chan, self.kernel_size, "gLN", is2d=self.is2d
        )
...
class LSTMFusion(FusionBasemodule):
    def __init__(
        ...
        **kwargs,
        ...
        if video_fusion:
            self.video_lstm = ConvLSTMFusionCell(
                self.vin_chan,
                self.ain_chan,
                self.kernel_size,
                self.bidirectional,
        ...
        self.audio_lstm = ConvLSTMFusionCell(
            self.ain_chan,
            self.vin_chan,
            self.kernel_size,
            self.bidirectional,
...
class GRUFusion(FusionBasemodule):
    def __init__(
        ...
        **kwargs,
        ...
        if video_fusion:
            self.video_lstm = ConvGRUFusionCell(
                self.vin_chan,
                self.ain_chan,
                self.kernel_size,
                self.bidirectional,
        ...
        self.audio_lstm = ConvGRUFusionCell(
            self.ain_chan,
            self.vin_chan,
            self.kernel_size,
            self.bidirectional,
...
class ATTNFusion(FusionBasemodule):
    def __init__(
        ...
        **kwargs,
        ...
        if video_fusion:
            self.video_lstm = ATTNFusionCell(
                self.vin_chan, self.ain_chan, self.kernel_size, self.is2d
            )
        self.audio_lstm = ATTNFusionCell(
            self.ain_chan, self.vin_chan, self.kernel_size, self.is2d
        )
...
class MultiModalFusion(nn.Module):
    ...
    def __build_fusion_module(self, **kwargs):
        fusion_class = (
            globals().get(self.fusion_type) if self.fusion_repeats > 0 else nn.Identity
        )
    ...
    def forward(self, audio: torch.Tensor, video: torch.Tensor):
        ...
        for i in range(self.fusion_repeats):
            if i == 0:
                ...
                audio_fused, video_fused = self.get_fusion_block(i)(
                    audio_fused + audio_residual, video_fused + video_residual
                )
