...
class FRCNNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.upsampling_depth):
            ...
            y = torch.cat(
                (
                    self.fusion_layers[i][0](downsampled_outputs[i - 1])
                    if i - 1 >= 0
                    else torch.Tensor().to(x_enc.device),
                    downsampled_outputs[i],
                    F.interpolate(
                        downsampled_outputs[i + 1],
                        size=shape[-(len(shape) // 2) :],
                        mode="nearest",
                    )
        ...
        for i in range(1, len(x_fused)):
            x_fused[i] = F.interpolate(
                x_fused[i], size=shape[-(len(shape) // 2) :], mode="nearest"
            )
