...
from src.model.ctcnet.utils.torch_utils import pad_x_to_y, shape_reconstructed
from src.model.ctcnet.videosubnetwork import VideoBlock
...
class AudioVisual(nn.Module):
    ...
    def __init__(
        ...
        self.fusion_levels = (
            fusion_levels if fusion_levels is not None else list(range(self.an_repeats))
        )
        self.fusion_type = fusion_type

        # pre and post processing layers
        self.pre_a = nn.Sequential(
            normalizations.get(norm_type)(in_chan), nn.Conv1d(in_chan, bn_chan, 1, 1)
        )
        self.pre_v = nn.Conv1d(
            self.vout_chan, video_config["in_chan"], kernel_size=3, padding=1
        )
        ...
        self.audio_block = AudioSubnetwork(
            bn_chan, hid_chan, upsampling_depth, norm_type, act_type
        )
        self.audio_concat = nn.Sequential(
            nn.Conv1d(bn_chan, hid_chan, 1, 1, groups=bn_chan), nn.PReLU()
        )
        self.crossmodal_fusion = self._build_crossmodal_fusion(
            bn_chan, video_config["in_chan"]
        )
    ...
    def _build_crossmodal_fusion(self, ain_chan, vin_chan):
        ...
        if self.fusion_shared:
            ...
            return nn.ModuleList(
                [module(ain_chan, vin_chan) for _ in range(self.an_repeats)]
            )
...
class CTCNet(BaseAVEncoderMaskerDecoder):
    ...
    def __init__(
        ...
        **fb_kwargs,
        ...
        encoder = _Padder(
            encoder, upsampling_depth=upsampling_depth, kernel_size=kernel_size
        )
