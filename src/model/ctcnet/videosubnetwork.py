...
from src.model.ctcnet.layers.activations import get as get_act
from src.model.ctcnet.layers.normalizations import get as get_norm
...
class VisualSubnetwork(nn.Module):
    ...
    def __init__(
        ...
        self.proj = ConvNormAct(
            in_chan, out_chan, kernel_size=1, norm_type=norm_type, act_type=act_type
        )
        ...
        for i in range(self.depth):
            ...
            y = torch.cat(
                (
                    ...
                    output[i],
                    F.interpolate(output[i + 1], size=T, mode="nearest")
                    if i + 1 < self.depth
                    else torch.Tensor().to(x.device),
                ),
                dim=1,
    ...
    def _build_downsample_layers(self):
        ...
        out.append(
            ConvNormAct(
                ...
                kernel_size=self.kernel_size,
                groups=self.out_chan,
                padding=cal_padding(0, self.kernel_size, 1, 1),
                norm_type=self.norm_type,
                act_type=None,
            )
        )
        ...
        for _ in range(1, self.depth):
            out.append(
                ConvNormAct(
                    ...
                    groups=self.out_chan,
                    padding=cal_padding(0, self.kernel_size, 1, self.dilation),
                    norm_type=self.norm_type,
                    act_type=None,
                )
            )
